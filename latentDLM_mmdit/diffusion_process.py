...
#     elif config.model.diffusion_process == "latentDLM_mmdit":
...
class ContinuousDiffusion(NoiseSchedule):
    """Continuous diffusion for latent vectors (like DiT for images)."""
    
    def __init__(self, beta_min=0.0001, beta_max=0.02):
        super().__init__(None)  # No tokenizer needed
        self.beta_min = beta_min
        self.beta_max = beta_max
        self.steps = 1000
        
        # Precompute schedules
        self.betas = torch.linspace(beta_min, beta_max, self.steps)
        self.alphas = 1 - self.betas
        self.alpha_bars = torch.cumprod(self.alphas, dim=0)
        
        # Required by base class but not used
        self.log_gamma = torch.tensor(0.0)
        self.log_B = torch.tensor(-20.0)
        self.mask_id = -1
        self.vocab_size = -1
    
    def get_alpha_beta(self, t):
        """Get alpha and beta for continuous diffusion."""
        # t is in [0, 1], map to [0, steps-1]
        t_idx = (t * (self.steps - 1)).long().clamp(0, self.steps - 1)
    ...
    # Required abstract method implementations (dummy for continuous)
    def logits_at_t(self, features, t):
        raise NotImplementedError("Not used for continuous diffusion")
...
# Update get_noise_schedule function to include continuous
def get_noise_schedule(config, tokenizer):
    if config.model.type == "autoregressive":
        return None
    elif config.model.diffusion_process == "gidd":
        ...
        noise_schedule = MaskedDiffusion(tokenizer)
    elif config.model.diffusion_process == "continuous":
        noise_schedule = ContinuousDiffusion(
